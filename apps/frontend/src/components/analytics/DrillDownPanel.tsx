  ...
  return (
    <div aria-hidden={!open} className={`fixed inset-0 z-40 ${open ? '' : 'pointer-events-none'}`}>
      ...
      <div className={`absolute inset-0 bg-black/40 transition-opacity ${open ? 'opacity-100' : 'opacity-0'}`}
           onClick={onClose} />
      {/* Panel */}
      <aside className={`absolute right-0 top-0 h-full w-full sm:w-[520px] bg-white dark:bg-zinc-900 shadow-xl
        ...
        <header className="flex items-center justify-between px-4 py-3 border-b border-zinc-200 dark:border-zinc-800">
          ...
          <button onClick={onClose} className="px-2 py-1 rounded border border-zinc-300 dark:border-zinc-700 hover:bg-zinc-50 dark:hover:bg-zinc-800">
            Close
          </button>
        </header>
        <div className="h-[calc(100%-3rem)] overflow-y-auto p-4">
          ...
          {!loading && activities.map((activity) => (
            <div key={activity.id} className="mb-3 p-3 rounded-lg border border-zinc-200 dark:border-zinc-700">
          ...
          {!loading && activities.length > 0 && (
            <div className="mt-4 text-center">
              <a href={`/activities?${new URLSearchParams(derivedFilters as any).toString()}`}
