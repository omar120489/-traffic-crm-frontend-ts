...
export default function SavedViewsDropdown({
  ...
  useEffect(() => {
    let mounted = true;
    SavedViewsService.list().then((res) => {
      if (mounted) {
        setList(res);
        setLoading(false);
      }
  ...
  const all = useMemo(() => [...(list.default ?? []), ...(list.personal ?? [])], [list]);
  ...
  return (
    <div className="relative">
      ...
      {open && (
        <div
          role="menu"
          className="absolute z-40 mt-2 w-72 rounded-xl border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 shadow-lg p-2"
        >
          ...
          {!loading &&
            all.map((v) => (
              <div
                key={v.id}
                className="group flex items-center justify-between gap-2 px-2 py-1 rounded-lg hover:bg-zinc-50 dark:hover:bg-zinc-800"
              >
                <button
